...
class UserForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            self.add_error('email', ValidationError('existing email!'))
        return email
    ...
    def clean(self):
        ...
        if password1 or password2:
            if password1 != password2:
                self.add_error(
                    'password2', ValidationError("passwords don't match")
                )
    ...
    def clean_last_name(self):
        ...
        if last_name and len(last_name) < 2:
            raise ValidationError(
                'O sobrenome tem que ser maior que 2 caracteres'
            )
        return last_name
    ...
    def clean(self):
        ...
        if first_name and last_name:
            if first_name == last_name:
                self.add_error(
                    'last_name', 'O nome e sobrenome não podem ser iguais'
                )
